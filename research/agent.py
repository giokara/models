...
class UvfAgentCore(object):
  ...
  def log_probs(self, states, actions, state_reprs, contexts=None):
    ...
    batch_dims = [tf.shape(input=states)[0], tf.shape(input=states)[1]]
    contexts = self.tf_context.context_multi_transition_fn(
        contexts, states=tf.cast(state_reprs, dtype=tf.float32))
  ...
  def add_noise_fn(self, action_fn, stddev=1.0, debug=False,
    ...
    if global_step is not None:
      stddev *= tf.maximum(  # Decay exploration during training.
          tf.compat.v1.train.exponential_decay(1.0, global_step, 1e6, 0.8), 0.5)
    def noisy_action_fn(state, context=None):
      ...
      if debug:
        action = uvf_utils.tf_print(
            ...
            first_n=100)
      noise_dist = tf.compat.v1.distributions.Normal(tf.zeros_like(action),
  ...
  def begin_episode_ops(self, mode, action_fn=None, state=None):
    ...
    for _, action_var in sorted(self._action_vars.items()):
      ...
      all_ops.append(tf.compat.v1.assign(action_var, sample_action))
  ...
  def cond_begin_episode_op(self, cond, input_vars, mode, meta_action_fn):
    ...
    with tf.control_dependencies(input_vars):
      cond_begin_episode_op = tf.cond(pred=cond, true_fn=continue_fn, false_fn=begin_episode_fn)
...
class MetaAgentCore(UvfAgentCore):
  ...
  def actor_loss(self, states, actions, rewards, discounts,
    ...
    regularizer = self._actions_reg * tf.reduce_mean(
        input_tensor=tf.reduce_sum(input_tensor=tf.abs(actions[:, self._k:]), axis=-1), axis=0)
...
def state_preprocess_net(
  ...
  with slim.arg_scope(
      ...
      weights_initializer=tf.compat.v1.keras.initializers.VarianceScaling(
          scale=1.0/3.0, mode=('FAN_IN').lower(), distribution=("uniform" if True else "truncated_normal"))):
...
def action_embed_net(
  ...
  with slim.arg_scope(
    ...
    if states is not None:
      ...
      if zero_time:
        states *= tf.constant([1.] * (states.shape[-1] - 1) + [0.], dtype=states.dtype)
      actions = tf.concat([actions, tf.cast(states, dtype=tf.float32)], -1)
...
def huber(x, kappa=0.1):
  return (0.5 * tf.square(x) * tf.cast(tf.abs(x) <= kappa, dtype=tf.float32) +
          kappa * (tf.abs(x) - 0.5 * kappa) * tf.cast(tf.abs(x) > kappa, dtype=tf.float32)
          ) / kappa
...
class StatePreprocess(object):
  ...
  def __init__(self, trainable=False,
    ...
    self._ndims = ndims
    self._state_preprocess_net = tf.compat.v1.make_template(
        self.STATE_PREPROCESS_NET_SCOPE, state_preprocess_net,
        create_scope_now_=True)
    self._action_embed_net = tf.compat.v1.make_template(
  ...
  def loss(self, states, next_states, low_actions, low_states):
    batch_size = tf.shape(input=states)[0]
    ...
    probs *= tf.constant([1.0] * (d - 1) + [1.0 / (1 - 0.99)],
                         dtype=tf.float32)
    probs /= tf.reduce_sum(input_tensor=probs)
    index_dist = tf.compat.v1.distributions.Categorical(probs=probs, dtype=tf.int64)
    indices = index_dist.sample(batch_size)
    batch_size = tf.cast(batch_size, tf.int64)
    next_indices = tf.concat(
        [tf.range(batch_size, dtype=tf.int64)[:, None],
         (1 + indices[:, None]) % d], -1)
    new_next_states = tf.compat.v1.where(indices < d - 1,
                               tf.gather_nd(low_states, next_indices),
                               next_states)
    next_states = new_next_states

    embed1 = tf.cast(self._state_preprocess_net(states), dtype=tf.float32)
    embed2 = tf.cast(self._state_preprocess_net(next_states), dtype=tf.float32)
    action_embed = self._action_embed_net(
        tf.compat.v1.layers.flatten(low_actions), states=states)

    tau = 2.0
    fn = lambda z: tau * tf.reduce_sum(input_tensor=huber(z), axis=-1)
    all_embed = tf.compat.v1.get_variable('all_embed', [1024, int(embed1.shape[-1])],
                                initializer=tf.compat.v1.zeros_initializer())
    upd = all_embed.assign(tf.concat([all_embed[batch_size:], embed2], 0))
    with tf.control_dependencies([upd]):
      close = 1 * tf.reduce_mean(input_tensor=fn(embed1 + action_embed - embed2))
      prior_log_probs = tf.reduce_logsumexp(
          input_tensor=-fn((embed1 + action_embed)[:, None, :] - all_embed[None, :, :]),
          axis=-1) - tf.math.log(tf.cast(all_embed.shape[0], dtype=tf.float32))
      far = tf.reduce_mean(input_tensor=tf.exp(-fn((embed1 + action_embed)[1:] - embed2[:-1])
