...
class BaseLayer(tf.keras.layers.Layer):
  ...
  def quantize_parameter(self, tensor, num_bits=8):
    ...
    if self.parameters.quantize and self.parameters.mode in [TRAIN, EVAL]:
      ...
      batch_min = tf.minimum(tf.reduce_min(input_tensor=tensor), 0.0)
      batch_max = tf.maximum(tf.reduce_max(input_tensor=tensor), 0.0)
  ...
  def add_reg_loss(self, weight):
    if self.parameters.regularizer_scale > 0.0:
      reg_scale = tf.convert_to_tensor(value=self.parameters.regularizer_scale)
  ...
  def get_batch_dimension(self, tensor):
    return tensor.get_shape().as_list()[0] or tf.shape(input=tensor)[0]

  def inverse_normalizer(self, mask):
    return tf.math.reciprocal(tf.reduce_sum(input_tensor=mask))
