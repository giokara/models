...
class Baseline(object):
  ...
  def reshape_batched_inputs(self, all_obs, all_actions,
    ...
    """
    batch_size = tf.shape(input=all_obs[0])[1]
  ...
  def get_values(self, all_obs, all_actions, internal_policy_states,
    ...
    for depth in xrange(self.n_hidden_layers):
      with tf.compat.v1.variable_scope('value_layer%d' % depth):
        w = tf.compat.v1.get_variable('w', [input_dim, self.hidden_dim])
        inputs = tf.nn.tanh(tf.matmul(inputs, w))
        input_dim = self.hidden_dim

    w_v = tf.compat.v1.get_variable('w_v', [input_dim, 1],
...
class UnifiedBaseline(Baseline):
  ...
  def get_values(self, all_obs, all_actions, internal_policy_states,
    ...
    def f_transform(q_values, tau):
      max_q = tf.reduce_max(input_tensor=q_values, axis=-1, keepdims=True)
      return tf.squeeze(max_q, [-1]) + tau * tf.math.log(
          tf.reduce_sum(input_tensor=tf.exp((q_values - max_q) / tau), axis=-1))
