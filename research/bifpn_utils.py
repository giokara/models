  ...
  if use_native_resize_op:
    ...
    def resize_nearest_neighbor(image):
      ...
      return tf.image.resize(
          image, [image_shape[1] * scale, image_shape[2] * scale], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
...
class BiFPNCombineLayer(tf.keras.layers.Layer):
  ...
  def _combine_fast_attention(self, inputs):
    ...
    normalizer = tf.reduce_sum(input_tensor=weights_non_neg) + 0.0001
