...
class Encoder(tf.keras.layers.Layer):
  ...
  def call(self, token_ids, seq_length):
    ...
    if self.parameters.mode in [base_layers.PREDICT, base_layers.TFLITE]:
      mask_rank2 = tf.ones(tf.shape(input=input_embeds)[:-1], dtype=tf.float32)
      seq_length = tf.reduce_sum(input_tensor=mask_rank2, axis=1)
    else:
      mask_rank2 = tf.sequence_mask(
          seq_length, tf.shape(input=input_embeds)[1], dtype=tf.float32)
    ...
    if self.parameters.mode in [base_layers.PREDICT, base_layers.TFLITE]:
      mask_rank2 = tf.ones(tf.shape(input=gbst_output)[:-1], dtype=tf.float32)
      seq_length = tf.reduce_sum(input_tensor=mask_rank2, axis=1)
    else:
      seq_length = seq_length / self.gbst_downsample_rate
    mask_rank2 = tf.sequence_mask(
        seq_length, tf.shape(input=gbst_output)[1], dtype=tf.float32)
    inverse_normalizer = tf.math.reciprocal(tf.reduce_sum(input_tensor=mask_rank2))
    maskr3 = tf.expand_dims(mask_rank2, axis=2)
