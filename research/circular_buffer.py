...
class CircularBuffer(object):
  ...
  def _create_variables(self, tensors):
    with tf.compat.v1.variable_scope(self._scope):
      for name in tensors.keys():
        tensor = tensors[name]
        self._tensors[name] = tf.compat.v1.get_variable(
  ...
  def maybe_add(self, tensors, condition):
    ...
    def _add():
      ...
      current_pos = tf.math.floormod(num_adds_inc - 1, self._buffer_size)
      update_ops = []
      for name in self._tensors.keys():
        update_ops.append(
            tf.compat.v1.scatter_update(self._tensors[name], current_pos, tensors[name]))
  ...
  def gather(self, indices, keys=None):
    ...
    with tf.compat.v1.name_scope('Gather'):
      index_bound_assert = tf.Assert(
          tf.less(
              tf.cast(tf.reduce_max(input_tensor=indices), dtype=tf.int64),
  ...
  def gather_nstep(self, num_steps, indices, keys=None):
    ...
    with tf.compat.v1.name_scope('Gather'):
      index_bound_assert = tf.Assert(
          tf.less_equal(
              tf.cast(tf.reduce_max(input_tensor=indices) + num_steps, dtype=tf.int64),
              self.get_num_adds()),
          ['Trajectory indices go out of bounds.'])
      with tf.control_dependencies([index_bound_assert]):
        indices = tf.map_fn(
            lambda x: tf.math.floormod(tf.range(x, x + num_steps), self._buffer_size),
            indices,
            dtype=tf.int64)
  ...
  def get_position(self):
    ...
    """
    return tf.cond(pred=self.get_num_adds() < 1,
                   true_fn=lambda: self.get_num_adds() - 1,
                   false_fn=lambda: tf.math.floormod(self.get_num_adds() - 1, self._buffer_size))
