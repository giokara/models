...
def every_n_steps(agent,
  ...
  cond = tf.equal(tf.math.floormod(environment_steps, n), 0)
  return cond
...
def every_n_episodes(agent,
  ...
  cond = tf.logical_and(
      tf.logical_or(
          ant_fell,
          tf.equal(tf.math.floormod(num_episodes + 1, n), 0)),
      tf.equal(tf.math.floormod(environment_steps, steps_per_episode), 0))
  return cond
...
def failed_reset_after_n_episodes(agent,
  ...
  del agent, state, action, transition_type, num_episodes
  dist = tf.sqrt(
      tf.reduce_sum(input_tensor=tf.math.squared_difference(state, reset_state)) + epsilon)
  cond = tf.logical_and(
