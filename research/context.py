...
class Context(object):
  ...
  def _make_reward_fn(self, reward_fns_list, reward_weights):
    ...
    def reward_fn(*args, **kwargs):
      ...
      rewards = tf.add_n(
          [r * tf.cast(w, dtype=tf.float32) for r, w in zip(rewards_list, reward_weights)])
  ...
  def set_env_context_op(self, context, disable_unnormalizer=False):
    ...
    def set_context_func(*env_context_values):
      tf.compat.v1.logging.info('[set_env_context_op] Setting gym environment context.')
      # pylint: disable=protected-access
      self.gym_env.set_context(*env_context_values)
      return ret_val
      # pylint: enable=protected-access

    with tf.compat.v1.name_scope('set_env_context'):
      set_op = tf.compat.v1.py_func(set_context_func, context, tf.float32,
  ...
  def step(self, mode, agent=None, action_fn=None, **kwargs):
    ...
    if agent is None:
      ...
      if self._context_transition_fn is not None:
        ...
        ops += [tf.compat.v1.assign(var, value) for var, value in zip(self.vars, values)]
      ops.append(tf.compat.v1.assign_add(self.t, 1))  # increment timer
      return ops
    else:
      ...
      with tf.control_dependencies(ops):  # Step high level context before computing low level one.
        ...
        low_level_context = [
            tf.cond(pred=tf.equal(self.t % self.meta_action_every_n, 0),
                    true_fn=lambda: tf.cast(action_fn(next_state, context=None), tf.float32),
                    false_fn=lambda: values)]
        ops = [tf.compat.v1.assign(var, value)
               for var, value in zip(self.vars, low_level_context)]
        with tf.control_dependencies(ops):
          return [tf.compat.v1.assign_add(self.t, 1)]  # increment timer
        return ops
  ...
  def reset(self, mode, agent=None, action_fn=None, state=None):
    ...
    if agent is None:
      ...
      for _, context_vars in sorted(self.context_vars.items()):
        ops = [tf.compat.v1.assign(var, value) for var, value in zip(context_vars, values)]
      all_ops += ops
      all_ops.append(self.set_env_context_op(values))
      all_ops.append(tf.compat.v1.assign(self.t, 0))  # reset timer
      return all_ops
    else:
      ...
      for key, context_vars in sorted(self.context_vars.items()):
        ops += [tf.compat.v1.assign(var, tf.zeros_like(var)) for var, meta_var in
                zip(context_vars, agent.tf_context.context_vars[key])]

      ops.append(tf.compat.v1.assign(self.t, 0))  # reset timer
      return ops
