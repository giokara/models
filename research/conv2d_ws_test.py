...
class ConvolutionTest(tf.test.TestCase):
  ...
  def testCreateConvWithCollection(self):
    height, width = 7, 9
    images = tf.random.uniform((5, height, width, 3), seed=1)
    with tf.compat.v1.name_scope('fe'):
  ...
  def testReuseConvWithWD(self):
    ...
    with self.cached_session():
      images = tf.random.uniform((5, height, width, 3), seed=1)
      weight_decay = tf.keras.regularizers.l2(0.5 * (0.01))
  ...
  def testDynamicOutputSizeWithRateOneValidPadding(self):
    ...
    with self.cached_session():
      images = tf.compat.v1.placeholder(np.float32, [None, None, None, input_size[3]])
      output = conv2d_ws.conv2d(
  ...
  def testDynamicOutputSizeWithRateOneValidPaddingNCHW(self):
    if tf.test.is_gpu_available(cuda_only=True):
      ...
      with self.session(use_gpu=True):
        images = tf.compat.v1.placeholder(np.float32, [None, input_size[1], None, None])
        output = conv2d_ws.conv2d(
            images,
            num_filters, [3, 3],
            rate=1,
