...
def global_avg_pool(x, scope=None):
  ...
  with tf.compat.v1.name_scope(scope, 'global_avg_pool', [x]):
...
def zero_pad(inputs, in_filter, out_filter):
  ...
  outputs = tf.pad(tensor=inputs, paddings=[[0, 0], [0, 0], [0, 0],
...
def conv2d(inputs,
  ...
  with tf.compat.v1.variable_scope(scope, 'Conv', [inputs], reuse=reuse):
    ...
    outputs = tf.nn.conv2d(
        input=inputs, filters=weights, strides=strides, padding='SAME', data_format='NHWC')
    return outputs
