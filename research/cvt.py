...
tf.compat.v1.app.flags.DEFINE_string('mode', 'train', '"train" or "eval')
tf.compat.v1.app.flags.DEFINE_string('model_name', 'default_model',
...
def main():
  ...
  with tf.Graph().as_default() as graph:
    ...
    summary_writer = tf.compat.v1.summary.FileWriter(config.summaries_dir)
    checkpoints_saver = tf.compat.v1.train.Saver(max_to_keep=1)
    best_model_saver = tf.compat.v1.train.Saver(max_to_keep=1)
