...
class DdpgAgent(object):
  ...
  def __init__(self,
               ...
               td_errors_loss=tf.compat.v1.losses.huber_loss,
               dqda_clipping=0.,
    ...
    self._scope = tf.compat.v1.get_variable_scope().name
    self._actor_net = tf.compat.v1.make_template(
        self.ACTOR_NET_SCOPE, actor_net, create_scope_now_=True)
    self._critic_net = tf.compat.v1.make_template(
        self.CRITIC_NET_SCOPE, critic_net, create_scope_now_=True)
    self._target_actor_net = tf.compat.v1.make_template(
        self.TARGET_ACTOR_NET_SCOPE, actor_net, create_scope_now_=True)
    self._target_critic_net = tf.compat.v1.make_template(
  ...
  def sample_action(self, state, stddev=1.0):
    ...
    agent_action += tf.random.normal(tf.shape(input=agent_action)) * stddev
  ...
  def actor_loss(self, states):
    ...
    dqda = tf.gradients(ys=[q_values], xs=[actions])[0]
    dqda_unclipped = dqda
    if self._dqda_clipping > 0:
      dqda = tf.clip_by_value(dqda, -self._dqda_clipping, self._dqda_clipping)

    actions_norm = tf.norm(tensor=actions)
    if self._debug_summaries:
      with tf.compat.v1.name_scope('dqda'):
        tf.compat.v1.summary.scalar('actions_norm', actions_norm)
        tf.compat.v1.summary.histogram('dqda', dqda)
        tf.compat.v1.summary.histogram('dqda_unclipped', dqda_unclipped)
        tf.compat.v1.summary.histogram('actions', actions)
        for a in range(self._num_action_dims):
          tf.compat.v1.summary.histogram('dqda_unclipped_%d' % a, dqda_unclipped[:, a])
          tf.compat.v1.summary.histogram('dqda_%d' % a, dqda[:, a])
  ...
  def critic_function(self, critic_values, states, weights=None):
    ...
    if weights is not None:
      weights = tf.convert_to_tensor(value=weights, dtype=critic_values.dtype)
      critic_values *= weights
    if critic_values.shape.ndims > 1:
      critic_values = tf.reduce_sum(input_tensor=critic_values,
                                    axis=range(1, critic_values.shape.ndims))
...
class TD3Agent(DdpgAgent):
  ...
  def __init__(self,
    ...
    self._critic_net = tf.compat.v1.make_template(
        self.CRITIC_NET_SCOPE, critic_net, create_scope_now_=True)
    self._critic_net2 = tf.compat.v1.make_template(
        self.CRITIC_NET2_SCOPE, critic_net, create_scope_now_=True)
    self._target_actor_net = tf.compat.v1.make_template(
        self.TARGET_ACTOR_NET_SCOPE, actor_net, create_scope_now_=True)
    self._target_critic_net = tf.compat.v1.make_template(
        self.TARGET_CRITIC_NET_SCOPE, critic_net, create_scope_now_=True)
    self._target_critic_net2 = tf.compat.v1.make_template(
  ...
  def target_value_net(self, states, for_critic_loss=False):
    ...
    noise = tf.clip_by_value(
        tf.random.normal(tf.shape(input=target_actions), stddev=0.2), -0.5, 0.5)
...
def gen_debug_td_error_summaries(
  ...
  """
  with tf.compat.v1.name_scope('td_errors'):
    tf.compat.v1.summary.histogram('td_targets', td_targets)
    tf.compat.v1.summary.histogram('q_values', q_values)
    tf.compat.v1.summary.histogram('target_q_values', target_q_values)
    tf.compat.v1.summary.histogram('td_errors', td_errors)
    with tf.compat.v1.name_scope('td_targets'):
      tf.compat.v1.summary.scalar('mean', tf.reduce_mean(input_tensor=td_targets))
      tf.compat.v1.summary.scalar('max', tf.reduce_max(input_tensor=td_targets))
      tf.compat.v1.summary.scalar('min', tf.reduce_min(input_tensor=td_targets))
    with tf.compat.v1.name_scope('q_values'):
      tf.compat.v1.summary.scalar('mean', tf.reduce_mean(input_tensor=q_values))
      tf.compat.v1.summary.scalar('max', tf.reduce_max(input_tensor=q_values))
      tf.compat.v1.summary.scalar('min', tf.reduce_min(input_tensor=q_values))
    with tf.compat.v1.name_scope('target_q_values'):
      tf.compat.v1.summary.scalar('mean', tf.reduce_mean(input_tensor=target_q_values))
      tf.compat.v1.summary.scalar('max', tf.reduce_max(input_tensor=target_q_values))
      tf.compat.v1.summary.scalar('min', tf.reduce_min(input_tensor=target_q_values))
    with tf.compat.v1.name_scope('td_errors'):
      tf.compat.v1.summary.scalar('mean', tf.reduce_mean(input_tensor=td_errors))
      tf.compat.v1.summary.scalar('max', tf.reduce_max(input_tensor=td_errors))
      tf.compat.v1.summary.scalar('min', tf.reduce_min(input_tensor=td_errors))
      tf.compat.v1.summary.scalar('mean_abs', tf.reduce_mean(input_tensor=tf.abs(td_errors)))
