...
def critic_net(states, actions,
  ...
  with slim.arg_scope(
    ...
    orig_states = tf.cast(states, dtype=tf.float32)
    #states = tf.to_float(states)
    states = tf.concat([tf.cast(states, dtype=tf.float32), tf.cast(actions, dtype=tf.float32)], -1)  #TD3
    ...
    if not for_critic_loss and num_reward_dims > 1:
      value = tf.reduce_sum(
          input_tensor=value * tf.abs(orig_states[:, -num_reward_dims:]), axis=-1)
  return value
