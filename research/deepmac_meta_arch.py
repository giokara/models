...
def subsample_instances(classes, weights, boxes, masks, num_subsamples):
  ...
  num_instances = tf.reduce_sum(input_tensor=tf.cast(weights > 0.5, tf.int32))
...
def resize_instance_masks(masks, shape):
  batch_size, num_instances = tf.shape(input=masks)[0], tf.shape(input=masks)[1]
  return tf.cond(
      pred=tf.shape(input=masks)[1] == 0,
      true_fn=lambda: tf.zeros((batch_size, num_instances, shape[0], shape[1])),
      false_fn=lambda: _resize_instance_masks_non_empty(masks, shape))
...
def per_instance_no_class_overlap(classes, boxes, height, width):
  ...
  per_class_instance_count = tf.reduce_sum(input_tensor=per_class_box_mask, axis=1)
  per_class_valid_map = per_class_instance_count < 2
  class_indices = tf.argmax(input=classes, axis=2)
...
def embedding_projection(x, y):
  ...
  dot = tf.reduce_sum(input_tensor=x * y, axis=3, keepdims=True)
  return  dot
...
def generate_2d_neighbors(input_tensor, dilation=2):
  ...
  input_tensor = tf.transpose(a=input_tensor, perm=(0, 3, 1, 2))
  ...
  # return: [8, B, H, W, C]
  return tf.transpose(a=output, perm=[4, 0, 2, 3, 1])


def gaussian_pixel_similarity(a, b, theta):
  norm_difference = tf.linalg.norm(tensor=a - b, axis=-1)
...
def dilated_cross_same_mask_label(instance_masks, dilation=2):
  ...
  instance_masks = tf.transpose(a=instance_masks, perm=(0, 2, 3, 1))
  ...
  return tf.transpose(a=same_mask_prob, perm=(0, 1, 4, 2, 3))
...
def _per_pixel_single_conv(input_tensor, params, channels):
  ...
  num_instances = tf.shape(input=params)[0]
...
class MaskHeadNetwork(tf.keras.layers.Layer):
  ...
  def __call__(self, instance_embedding, pixel_embedding, training):
    ...
    height = tf.shape(input=pixel_embedding)[1]
    width = tf.shape(input=pixel_embedding)[2]
...
class DeepMACMetaArch(center_net_meta_arch.CenterNetMetaArch):
  ...
  def _get_mask_head_input(self, boxes, pixel_embedding):
    ...
    if self._deepmac_params.predict_full_resolution_masks:
      ...
      mask_shape = tf.shape(input=pixel_embeddings_processed)
  ...
  def _get_instance_embeddings(self, boxes, instance_embedding):
    ...
    output_height = tf.cast(tf.shape(input=instance_embedding)[1], tf.float32)
    output_width = tf.cast(tf.shape(input=instance_embedding)[2], tf.float32)
    ymin = boxes[:, :, 0]
    xmin = boxes[:, :, 1]
    ymax = boxes[:, :, 2]
    xmax = boxes[:, :, 3]
  ...
  def _predict_deaugmented_mask_logits_on_augmented_inputs(
    ...
    batch_size = tf.shape(input=preprocessed_inputs)[0]
  ...
  def _resize_logits_like_gt(self, logits, gt):
    height, width = tf.shape(input=gt)[2], tf.shape(input=gt)[3]
  ...
  def _aggregate_classification_loss(self, loss, gt, pred, method):
    ...
    if method == 'normalize_auto':
      normalization = 1.0
      if rank == 2:
        return tf.reduce_sum(input_tensor=loss, axis=axes)
      else:
        return tf.reduce_mean(input_tensor=loss, axis=axes)

    elif method == 'normalize_groundtruth_count':
      normalization = tf.reduce_sum(input_tensor=gt, axis=axes)
      return tf.reduce_sum(input_tensor=loss, axis=axes) / normalization
      ...
      normalization = (
          (gt * tf.reduce_sum(input_tensor=gt, keepdims=True, axis=axes)) +
          (1 - gt) * tf.reduce_sum(input_tensor=1 - gt, keepdims=True, axis=axes))
      return tf.reduce_sum(input_tensor=loss / normalization, axis=axes)
  ...
  def _compute_mask_prediction_loss(
    ...
    height, width = tf.shape(input=mask_logits)[2], tf.shape(input=mask_logits)[3]

    if self._deepmac_params.ignore_per_class_box_overlap:
      mask_logits *= per_instance_no_class_overlap(
          classes, boxes, height, width)

      height, wdith = tf.shape(input=mask_gt)[2], tf.shape(input=mask_gt)[3]
      mask_logits *= per_instance_no_class_overlap(
  ...
  def _compute_box_consistency_loss(
    ...
    shape = tf.shape(input=mask_logits)
    ...
    for axis in [2, 3]:
      ...
      if self._deepmac_params.box_consistency_tightness:
        pred_max_raw = tf.reduce_max(input_tensor=pred_crop, axis=axis)
        pred_max_within_box = tf.reduce_max(input_tensor=pred_crop * gt_crop, axis=axis)
        box_1d = tf.reduce_max(input_tensor=gt_crop, axis=axis)
        pred_max = ((box_1d * pred_max_within_box) +
                    ((1 - box_1d) * pred_max_raw))

      else:
        pred_max = tf.reduce_max(input_tensor=pred_crop, axis=axis)

      pred_max = pred_max[:, :, :, tf.newaxis]
      gt_max = tf.reduce_max(input_tensor=gt_crop, axis=axis)[:, :, :, tf.newaxis]
  ...
  def _compute_color_consistency_loss(
    ...
    if not self._deepmac_params.predict_full_resolution_masks:
      ...
      return tf.zeros(tf.shape(input=boxes)[:2])

    dilation = self._deepmac_params.color_consistency_dilation

    height, width = (tf.shape(input=preprocessed_image)[1],
                     tf.shape(input=preprocessed_image)[2])
    ...
    loss = tf.reduce_sum(input_tensor=per_pixel_loss, axis=[0, 3, 4])
    num_box_pixels = tf.maximum(1.0, tf.reduce_sum(input_tensor=box_mask, axis=[2, 3]))
  ...
  def _self_supervision_loss(
    ...
    original_shape = tf.shape(input=predicted_logits)
    ...
    num_box_pixels = tf.reduce_sum(input_tensor=weights, axis=[1, 2])
    ...
    if loss_name == 'loss_dice':
      ...
      diff_sq_sum = tf.reduce_sum(input_tensor=diff_sq * weights, axis=[1, 2])
      ...
      agg_loss = tf.reduce_sum(input_tensor=loss * weights, axis=[1, 2]) / num_box_pixels
    else:
  ...
  def _compute_self_supervised_augmented_loss(
    ...
    if deaugmented_logits is None:
      ...
      return tf.zeros(tf.shape(input=original_logits)[:2])
  ...
  def _compute_pointly_supervised_loss_from_keypoints(
    ...
    if keypoints_gt is None:
      ...
      return tf.zeros(tf.shape(input=mask_logits)[:2])
    ...
    num_keypoints = tf.shape(input=keypoints_gt)[2]
    keypoints_nan = tf.math.is_nan(keypoints_gt)
    keypoints_gt = tf.compat.v1.where(
        keypoints_nan, tf.zeros_like(keypoints_gt), keypoints_gt)
    weights = tf.cast(
        tf.logical_not(tf.reduce_any(input_tensor=keypoints_nan, axis=3)), tf.float32)
    ...
    weights_flat = tf.logical_not(
        tf.reduce_any(input_tensor=keypoints_nan, axis=2))
  ...
  def _compute_masks_loss(self, prediction_dict):
    ...
    prediction_shape = tf.shape(input=prediction_dict[MASK_LOGITS_GT_BOXES][0])
    ...
    for (mask_logits, self_supervised_mask_logits) in zip(
      ...
      num_instances = tf.maximum(tf.reduce_sum(input_tensor=gt_weights), 1.0)
      num_instances_allowed = tf.maximum(
          tf.reduce_sum(input_tensor=valid_mask_weights), 1.0)

      loss_dict[DEEP_MASK_ESTIMATION] += (
          tf.reduce_sum(input_tensor=sample_loss_dict[DEEP_MASK_ESTIMATION]) /
          num_instances_allowed)

      for loss_name in WEAK_LOSSES:
        loss_dict[loss_name] += (tf.reduce_sum(input_tensor=sample_loss_dict[loss_name]) /
                                 num_instances)
  ...
  def _postprocess_masks(self, boxes_output_stride,
    ...
    height, width = (tf.shape(input=instance_embedding)[1],
                     tf.shape(input=instance_embedding)[2])
    ...
    if self._deepmac_params.predict_full_resolution_masks:
      ...
      height, width = tf.shape(input=mask_logits)[1], tf.shape(input=mask_logits)[2]
      height *= self._stride
      width *= self._stride
