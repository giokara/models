...
class DepparseModule(task_module.SemiSupervisedModule):
  def __init__(self, config, task_name, n_classes, inputs, encoder):
    ...
    self.labels = labels = tf.compat.v1.placeholder(tf.float32, [None, None, None],
                                          name=task_name + '_labels')

    class PredictionModule(object):
      def __init__(self, name, dep_reprs, head_reprs, roll_direction=0):
        self.name = name
        with tf.compat.v1.variable_scope(name + '/predictions'):
          ...
          arc_head_hidden = tf.nn.dropout(arc_head_hidden, rate=1 - (inputs.keep_prob))
          arc_dep_hidden = tf.nn.dropout(arc_dep_hidden, rate=1 - (inputs.keep_prob))

          # bilinear classifier excluding the final dot product
          arc_head = tf.compat.v1.layers.dense(
              arc_head_hidden, config.depparse_projection_size, name='arc_head')
          W = tf.compat.v1.get_variable('shared_W',
                              shape=[config.projection_size, n_classes,
                                     config.depparse_projection_size])
          Wr = tf.compat.v1.get_variable('relation_specific_W',
          ...
          scale = tf.compat.v1.get_variable('scale', initializer=scale, dtype=tf.float32)
          arc_dep /= scale
          arc_head /= scale

          # compute the scores for each candidate arc
          word_scores = tf.matmul(arc_head, arc_dep, transpose_b=True)
          root_scores = tf.compat.v1.layers.dense(arc_head, n_classes, name='root_score')
    ...
    self.preds = tf.argmax(input=primary.logits, axis=-1)
