...
class DetectionBoxEmbedder(Embedder):
  ...
  def build(self, observations):
    ...
    """
    with tf.compat.v1.variable_scope(self._scope, default_name=''):
      shape = observations[0].shape
      dets = tf.reshape(observations[0], [-1, shape[-2], shape[-1]])
      det_num = tf.reshape(observations[1], [-1])
      lstm_cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(self._rnn_state_size)
      batch_size = tf.shape(input=dets)[0]
...
class ResNet(Embedder):
  ...
  def _build_model(self, images):
    ...
    with tf.compat.v1.variable_scope('pool_logit'):
  ...
  def _residual(self,
    ...
    with tf.compat.v1.variable_scope('sub_add'):
      if in_filter != out_filter:
        ...
        orig_x = tf.pad(
            tensor=orig_x, paddings=[[0, 0], [0, 0], [0, 0], [(out_filter - in_filter) // 2,
...
class MLPEmbedder(Embedder):
  ...
  def build(self, features):
    ...
    for i, dim in enumerate(self._layers):
      with tf.compat.v1.variable_scope('layer_%i' % i):
  ...
  def _fully_connected(self, x, out_dim):
    w = tf.compat.v1.get_variable(
        'DW', [x.get_shape()[1], out_dim],
        initializer=tf.compat.v1.variance_scaling_initializer(distribution='uniform'))
    b = tf.compat.v1.get_variable(
        'biases', [out_dim], initializer=tf.compat.v1.constant_initializer())
    return tf.compat.v1.nn.xw_plus_b(x, w, b)
...
class SmallNetworkEmbedder(Embedder):
  ...
  def build(self, images):
    ...
    with slim.arg_scope(
        ...
        weights_regularizer=tf.keras.regularizers.l2(0.5 * (self._params.weight_decay_rate)),
        biases_initializer=tf.compat.v1.zeros_initializer()):
      with slim.arg_scope([slim.conv2d], padding='SAME'):
        # convert the image to one hot if needed.
        if self._params.to_one_hot:
          net = tf.one_hot(
              tf.squeeze(tf.cast(images, dtype=tf.int32), axis=[-1]),
