...
def restore_from_checkpoint(sess, saver):
  ...
  if not ckpt or not ckpt.model_checkpoint_path:
    tf.compat.v1.logging.info('No checkpoint found at %s', FLAGS.checkpoint_dir)
    return False
...
def run_eval(eval_ops, summary_writer, saver):
  ...
  with sv.managed_session(
    ...
    tf.compat.v1.logging.info('Running %d batches for evaluation.', num_batches)
    for i in range(num_batches):
      if (i + 1) % 10 == 0:
        tf.compat.v1.logging.info('Running batch %d/%d...', i + 1, num_batches)
      if (i + 1) % 50 == 0:
...
def _log_values(sess, value_ops, summary_writer=None):
  ...
  tf.compat.v1.logging.info('Eval metric values:')
  summary = tf.compat.v1.summary.Summary()
  for name, val in zip(metric_names, values):
    summary.value.add(tag=name, simple_value=val)
    tf.compat.v1.logging.info('%s = %.3f', name, val)

  if summary_writer is not None:
    global_step_val = sess.run(tf.compat.v1.train.get_global_step())
    tf.compat.v1.logging.info('Finished eval for step ' + str(global_step_val))
...
def main(_):
  ...
  tf.compat.v1.logging.info('Building eval graph...')
  output = graphs.get_model().eval_graph(FLAGS.eval_data)
  eval_ops, moving_averaged_variables = output

  saver = tf.compat.v1.train.Saver(moving_averaged_variables)
  summary_writer = tf.compat.v1.summary.FileWriter(
      FLAGS.eval_dir, graph=tf.compat.v1.get_default_graph())
