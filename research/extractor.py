...
def MakeExtractor(config):
  ...
  # Load model.
  model = tf.compat.v1.saved_model.load(config.model_path)

  # Input image scales to use for extraction.
  image_scales_tensor = tf.convert_to_tensor(value=list(config.image_scales))
  ...
  def ExtractorFn(image, resize_factor=1.0):
    ...
    # Input tensors.
    image_tensor = tf.convert_to_tensor(value=resized_image)
    ...
    if config.use_global_features:
      ...
      unnormalized_global_descriptor = tf.reduce_sum(
          input_tensor=global_descriptors_per_scale, axis=0, name='sum_pooling')
