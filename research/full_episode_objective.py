...
class Reinforce(objective.Objective):
  ...
  def get(self, rewards, pads, values, final_values,
    ...
    seq_length = tf.shape(input=rewards)[0]
    ...
    total_rewards = tf.reduce_sum(input_tensor=rewards, axis=0)
    total_log_probs = tf.reduce_sum(input_tensor=log_probs, axis=0)

    rewards_and_bonus = (total_rewards +
                         self.bonus_weight *
                         self.get_bonus(total_rewards, total_log_probs))

    baseline = tf.reduce_mean(input_tensor=rewards_and_bonus, axis=1, keepdims=True)
    ...
    tf.compat.v1.summary.histogram('log_probs', total_log_probs)
    tf.compat.v1.summary.histogram('rewards', total_rewards)
    tf.compat.v1.summary.scalar('avg_rewards',
                      tf.reduce_mean(input_tensor=total_rewards))
    tf.compat.v1.summary.scalar('loss', loss)

    return loss, raw_loss, baseline, gradient_ops, tf.compat.v1.summary.merge_all()
