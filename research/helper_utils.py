...
def setup_loss(logits, labels):
  ...
  cost = tf.compat.v1.losses.softmax_cross_entropy(onehot_labels=labels,
                                         logits=logits)
...
def eval_child_model(session, model, data_loader, mode):
  ...
  tf.compat.v1.logging.info('model.batch_size is {}'.format(model.batch_size))
...
def run_epoch_training(session, model, data_loader, curr_epoch):
  ...
  tf.compat.v1.logging.info('steps per epoch: {}'.format(steps_per_epoch))
  ...
  tf.compat.v1.logging.info('lr of {} for epoch {}'.format(curr_lr, curr_epoch))
  ...
  for step in xrange(steps_per_epoch):
    ...
    if step % 20 == 0:
      tf.compat.v1.logging.info('Training {}/{}'.format(step, steps_per_epoch))
  ...
  tf.compat.v1.logging.info('Train accuracy: {}'.format(train_accuracy))
