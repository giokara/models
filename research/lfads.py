...
class LFADS(object):
  ...
  def __init__(self, hps, kind="train", datasets=None):
    ...
    # Dropout the data.
    dataset_do_bxtxd = tf.nn.dropout(tf.cast(dataset_ph, dtype=tf.float32), rate=1 - (keep_prob))
    if hps.ext_input_dim > 0:
      ext_input_do_bxtxi = tf.nn.dropout(ext_input_bxtxi, rate=1 - (keep_prob))
    else:
    ...
    with tf.compat.v1.variable_scope("z", reuse=False):
      ...
      if ic_dim > 0:
        ...
        ic_enc = tf.nn.dropout(ic_enc, rate=1 - (keep_prob))
    ...
    for t in range(num_steps):
      ...
      if co_dim > 0:
        ...
        con_in_t = tf.nn.dropout(con_in_t, rate=1 - (keep_prob))
      ...
      with tf.compat.v1.variable_scope("gen", reuse=True if t > 0 else None):
        ...
        gen_outs[t] = tf.nn.dropout(gen_outs[t], rate=1 - (keep_prob))
