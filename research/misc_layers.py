  ...
  def call(self, keys, queries, sequence_length):
    ...
    if self.parameters.mode not in [base_layers.PREDICT, base_layers.TFLITE]:
      sequence_mask = tf.sequence_mask(
          sequence_length, maxlen=tf.shape(input=keys)[1], dtype=tf.float32)
...
class GBSTLayerV2(base_layers.BaseLayer):
  ...
  def call(self, inputs, seq_length):
    ...
    if self.parameters.mode in [base_layers.PREDICT, base_layers.TFLITE]:
      num_steps = tf.shape(input=inputs)[1]
    ...
    for subword_len in self.subword_blocks_width:
      ...
      candidates = tf.nn.avg_pool2d(
          input=inputs, ksize=[subword_len], strides=[subword_len], padding="SAME")
    ...
    output = self.qoutput(tf.reduce_mean(input_tensor=dot_product, axis=-1, keepdims=True))
    ...
    if self.downsample_rate > 1:
      output = tf.nn.avg_pool2d(
          input=output, ksize=(self.downsample_rate,),
