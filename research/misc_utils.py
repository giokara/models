...
def random_substr(str_tensor, max_words):
  ...
  word_batch_r = tf.strings.split(str_tensor).to_sparse()
  ...
  max_words_batch = tf.reduce_max(input_tensor=words_per_example)
  rnd = tf.random.uniform(
      tf.shape(input=start_idx), minval=0, maxval=max_words_batch, dtype=tf.int64)
  off_start_idx = tf.math.floormod(rnd, max_val)
  new_words_per_example = tf.compat.v1.where(
  ...
  indices = tf.expand_dims(tf.range(tf.size(input=words), dtype=tf.int64), axis=0)
  ...
  keep_indices = tf.reduce_any(input_tensor=within_limit, axis=0)
  ...
  return tf.strings.reduce_join(inputs=new_tensor, axis=1, separator=" ")
