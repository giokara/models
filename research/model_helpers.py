...
def project(input_layers, size, name='projection'):
  return tf.add_n([tf.compat.v1.layers.dense(layer, size, name=name + '_' + str(i))
                   for i, layer in enumerate(input_layers)])


def lstm_cell(cell_size, keep_prob, num_proj):
  return tf.contrib.rnn.DropoutWrapper(
      tf.compat.v1.nn.rnn_cell.LSTMCell(cell_size, num_proj=min(cell_size, num_proj)),
      output_keep_prob=keep_prob)


def multi_lstm_cell(cell_sizes, keep_prob, num_proj):
  return tf.compat.v1.nn.rnn_cell.MultiRNNCell([lstm_cell(cell_size, keep_prob, num_proj)
...
def masked_ce_loss(logits, labels, mask, sparse=False, roll_direction=0):
  ...
  return tf.reduce_sum(input_tensor=mask * ce) / tf.cast(tf.reduce_sum(input_tensor=mask), dtype=tf.float32)
