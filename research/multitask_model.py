...
class Inference(object):
  def __init__(self, config, inputs, pretrained_embeddings, tasks):
    ...
    self.modules = {}
    for task in tasks:
      with tf.compat.v1.variable_scope(task.name):
...
class Model(object):
  def __init__(self, config, pretrained_embeddings, tasks):
    ...
    with tf.compat.v1.variable_scope('model', reuse=tf.compat.v1.AUTO_REUSE) as scope:
      ...
      if config.ema_test or config.ema_teacher:
        ...
        model_vars = tf.compat.v1.get_collection("trainable_variables", "model")
  ...
  def _get_optimizer(self):
    global_step = tf.compat.v1.get_variable('global_step', initializer=0, trainable=False)
    warm_up_multiplier = (tf.minimum(tf.cast(global_step, dtype=tf.float32),
                                     self._config.warm_up_steps)
                          / self._config.warm_up_steps)
    decay_multiplier = 1.0 / (1 + self._config.lr_decay *
                              tf.sqrt(tf.cast(global_step, dtype=tf.float32)))
    lr = self._config.lr * warm_up_multiplier * decay_multiplier
    optimizer = tf.compat.v1.train.MomentumOptimizer(lr, self._config.momentum)
