...
class Objective(object):
  ...
  def training_ops(self, loss, learning_rate=None):
    ...
    if self.clip_norm:
      ...
      tf.compat.v1.summary.scalar('grad_global_norm', global_norm)
...
def discounted_two_sided_sum(values, discount, rollout):
  ...
  expanded_values = tf.concat(
      [tf.zeros([rollout - 1, tf.shape(input=values)[1]]), values,
       tf.zeros([rollout - 1, tf.shape(input=values)[1]])], 0)
...
class ActorCritic(Objective):
  ...
  def __init__(self, learning_rate, clip_norm=5,
    ...
    self.eps_lambda = tf.compat.v1.get_variable(  # TODO: need a better way
        'eps_lambda', [], initializer=tf.compat.v1.constant_initializer(eps_lambda),
        trainable=False)
    self.new_eps_lambda = tf.compat.v1.placeholder(tf.float32, [])
  ...
  def get(self, rewards, pads, values, final_values,
    ...
    tf.compat.v1.summary.scalar('raw_loss', raw_loss)
