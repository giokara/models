...
class LbfgsOptimization(object):
  ...
  def setup_placeholders(self):
    ...
    self.intended_values = tf.compat.v1.placeholder(tf.float32, [None], 'intended_values')
  ...
  def setup(self, var_list, values, targets, pads,
    ...
    self.raw_loss = (tf.reduce_sum(input_tensor=(1 - pads) * tf.square(values - self.intended_values))
                     / tf.reduce_sum(input_tensor=1 - pads))
...
class GradOptimization(object):
  ...
  def setup(self, var_list, values, targets, pads,
    ...
    params = var_list
    grads = tf.gradients(ys=self.raw_loss, xs=params)
...
class BestFitOptimization(object):
  ...
  def setup_placeholders(self):
    self.new_regression_weight = tf.compat.v1.placeholder(
  ...
  def setup(self, var_list, values, targets, pads,
    ...
    self.update_regression_weight = tf.compat.v1.assign(
