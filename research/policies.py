...
class LSTMPolicy(Policy):
  ...
  def _build_lstm(self, encoded_inputs, prev_state, episode_length,
    ...
    with tf.compat.v1.variable_scope('LSTM'):
      lstm_cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(self._lstm_state_size)
      if prev_state is None:
        ...
        tf_prev_state = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(prev_state[0],
                                                      prev_state[1])
  ...
  def build(
    ...
    with tf.compat.v1.variable_scope('policy'):
      with slim.arg_scope(
          ...
          weights_regularizer=tf.keras.regularizers.l2(0.5 * (self._weight_decay))):
        all_inputs = []

        # Concatenating the embedding of each modality by applying the embedders
        # to corresponding observations.
        def embed(name):
          with tf.compat.v1.variable_scope('embed_{}'.format(name)):
        ...
        with tf.compat.v1.variable_scope('embed_goal'):
...
class ReactivePolicy(TaskPolicy):
  ...
  def build(self, feeds, prev_state):
    ...
    with tf.compat.v1.variable_scope('output_decoder'):
...
class RNNPolicy(TaskPolicy):
  ...
  def build(self, feeds, state):
    ...
    cell = lambda: tf.compat.v1.nn.rnn_cell.BasicLSTMCell(params.cell_size)
