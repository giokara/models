...
class GeMPooling2D(tf.keras.layers.Layer):
  ...
  def call(self, x):
    ...
    tmp = tf.nn.avg_pool2d(input=tmp, ksize=self.pool_size, strides=self.strides, padding=self.padding,
...
def mac(x, axis=None):
  ...
  if axis is None:
    axis = [1, 2]
  return tf.reduce_max(input_tensor=x, axis=axis, keepdims=False)
...
def spoc(x, axis=None):
  ...
  if axis is None:
    axis = [1, 2]
  return tf.reduce_mean(input_tensor=x, axis=axis, keepdims=False)
...
def gem(x, axis=None, power=3., eps=1e-6):
  ...
  out = tf.pow(tf.reduce_mean(input_tensor=tmp, axis=axis, keepdims=False), 1. / power)
  return out
