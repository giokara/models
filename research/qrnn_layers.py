...
class QRNNUnidirectionalPoolingCore(base_layers.BaseLayer):
  ...
  def _qrnn_pooling(self, multipler, constant):
    ...
    gate_shape = tf.shape(input=multipler)
    ...
    multiplier_transposed = tf.transpose(a=multipler, perm=[1, 0, 2])
    constant_transposed = tf.transpose(a=constant, perm=[1, 0, 2])
    ...
    if dynamic_loop:
      ...
      _, state, states = tf.while_loop(cond=_termination, body=_step,
                                       loop_vars=[index, state, states])
      states = states.stack()
    else:
    ...
    return tf.transpose(a=states, perm=[1, 0, 2])
...
class QRNNUnidirectionalPooling(base_layers.BaseLayer):
  ...
  def _qrnn_zoneout(self, multipler, constant):
    ...
    if enable_zoneout and self.parameters.mode == base_layers.TRAIN:
      ...
      zoneout_mask = tf.random.uniform(tf.shape(input=multipler), maxval=1.0)
...
class QRNNUnidirectional(base_layers.BaseLayer):
  ...
  def call(self, inputs, mask, inverse_normalizer=None):
    if inverse_normalizer is None:
      inverse_normalizer = tf.math.reciprocal(tf.reduce_sum(input_tensor=mask))
