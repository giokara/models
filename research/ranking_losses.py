...
def contrastive_loss(queries, positives, negatives, margin=0.7, eps=1e-6):
  ...
  Returns:
    loss: Scalar tensor.
  """
  dim = tf.shape(input=queries)[1]
  # Number of `queries`.
  batch_size = tf.shape(input=queries)[0]
  # Number of `positives`.
  np = tf.shape(input=positives)[0]
  # Number of `negatives`.
  num_neg = tf.shape(input=negatives)[1]
  ...
  distances = tf.norm(tensor=stacked_queries - positives_and_negatives + eps, axis=1)
  ...
  loss = tf.reduce_sum(input_tensor=tf.concat([positives_part, negatives_part], 0))
  return loss
...
def triplet_loss(queries, positives, negatives, margin=0.1):
  ...
  distance_positives = tf.reduce_sum(
      input_tensor=tf.square(stacked_queries - stacked_positives), axis=1)
  distance_negatives = tf.reduce_sum(
      input_tensor=tf.square(stacked_queries - stacked_negatives), axis=1)
  # Final triplet loss calculation.
  loss = tf.reduce_sum(
      input_tensor=tf.maximum(distance_positives - distance_negatives + margin, 0.0))
  return loss
