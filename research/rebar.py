...
class SBN(object):  # REINFORCE
  ...
  def __init__(self,
    ...
    self.n_samples = tf.compat.v1.placeholder('int32')
    self.x = tf.compat.v1.placeholder('float', [None, self.hparams.n_input])
    self._x = tf.tile(self.x, [self.n_samples, 1])

    self.batch_size = tf.shape(input=self._x)[0]
    ...
    self.prior = tf.Variable(tf.zeros([self.hparams.n_hidden],
                                      dtype=tf.float32),
                             name='p_prior',
                             collections=[tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, P_COLLECTION])
    ...
    self.optimizer_class = tf.compat.v1.train.AdamOptimizer(
  ...
  def _create_eta(self, shape=[], collection='CV'):
    return 2 * tf.sigmoid(tf.Variable(tf.zeros(shape), trainable=False,
                                      collections=[collection, tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, Q_COLLECTION]))
  ...
  def _reshape(self, t):
    return tf.transpose(a=tf.reshape(t,
                      [self.n_samples, -1]))
  ...
  def _create_train_op(self, grads_and_vars, extra_grads_and_vars=[]):
    ...
    if len(self.baseline_loss) > 0:
      mean_baseline_loss = tf.reduce_mean(input_tensor=tf.add_n(self.baseline_loss))
      extra_grads_and_vars += self.optimizer_class.compute_gradients(
          mean_baseline_loss,
          var_list=tf.compat.v1.get_collection('BASELINE'))

    # Ensure that all required tensors are computed before updates are executed
    extra_optimizer = tf.compat.v1.train.AdamOptimizer(
    ...
    with tf.control_dependencies(
      ...
      if self.eval_mode:
        grads_and_vars = [(g, v) for g, v in grads_and_vars
                          if v not in tf.compat.v1.get_collection(P_COLLECTION)]
    ...
    self.grad_variance = tf.reduce_mean(input_tensor=variance_estimator)

  def _create_network(self):
    logF = self._create_loss()
    self.optimizerLoss = tf.reduce_mean(input_tensor=self.optimizerLoss)
    ...
    self.iwae = tf.reduce_mean(input_tensor=U.logSumExp(self.logF, axis=1) -
                               tf.math.log(tf.cast(self.n_samples, dtype=tf.float32)))
  ...
  def _generate_randomness(self):
    for i in xrange(self.hparams.n_layer):
      self.uniform_samples[i] = tf.stop_gradient(tf.random.uniform(
  ...
  def _u_to_v(self, log_alpha, u, eps = 1e-8):
    ...
    v = tf.compat.v1.where(u > u_prime, v_1, v_0)
  ...
  def _random_sample(self, log_alpha, u, layer):
    ...
    samples = tf.stop_gradient(tf.cast(x > 0, dtype=tf.float32))
  ...
  def get_nvil_gradient(self):
    ...
    optimizerLoss = -(tf.stop_gradient(learning_signal)*logQHard +
                           reinforce_model_grad)
    optimizerLoss = tf.reduce_mean(input_tensor=optimizerLoss)
  ...
  def get_muprop_gradient(self):
    ...
    # Compute gradients
    muELBOGrads = tf.gradients(ys=tf.reduce_sum(input_tensor=muELBO),
                               xs=[ muSamples[i]['activation'] for
    ...
    for i in xrange(self.hparams.n_layer):
      dfDiff = tf.reduce_sum(
          input_tensor=muELBOGrads[i] * (hardSamples[i]['activation'] -
                            muSamples[i]['activation']),
          axis=1)
      dfMu = tf.reduce_sum(
          input_tensor=tf.stop_gradient(muELBOGrads[i]) *
  ...
  def multiply_by_eta_per_unit(self, h_grads, eta):
    ...
    for (g, v) in h_grads:
      if g is None:
        ...
        eta_statistics.extend(tf.nn.moments(x=tf.squeeze(eta[v]), axes=[0]))
    return res, eta_statistics
  ...
  def get_dynamic_rebar_gradient(self):
    ...
    f_grads = self.optimizer_class.compute_gradients(tf.reduce_mean(input_tensor=-nvil_gradient))

    eta = {}
    h_grads, eta_statistics = self.multiply_by_eta_per_layer(
        self.optimizer_class.compute_gradients(tf.reduce_mean(input_tensor=gumbel_cv)),
        eta)
    ...
    variance_objective = tf.reduce_mean(input_tensor=tf.square(g - gbar))

    reinf_g_t = 0
    if self.hparams.quadratic:
      for layer in xrange(self.hparams.n_layer):
        gumbel_learning_signal, _ = extra[layer]
        df_dt = tf.gradients(ys=gumbel_learning_signal, xs=tiled_pre_temperature)[0]
        reinf_g_t_i, _ = self.multiply_by_eta_per_layer(
            self.optimizer_class.compute_gradients(tf.reduce_mean(input_tensor=tf.stop_gradient(df_dt) * logQHard[layer])),
            eta)
      ...
      reinf_g_t, _ = self.multiply_by_eta_per_layer(
          self.optimizer_class.compute_gradients(tf.reduce_mean(input_tensor=tf.stop_gradient(df_dt) * tf.add_n(logQHard))),
          eta)
      reinf_g_t = U.vectorize(reinf_g_t, set_none_to_zero=True)

    reparam_g, _ = self.multiply_by_eta_per_layer(
        self.optimizer_class.compute_gradients(tf.reduce_mean(input_tensor=reparam)),
        eta)
    reparam_g = U.vectorize(reparam_g, set_none_to_zero=True)
    reparam_g_t = tf.gradients(ys=tf.reduce_mean(input_tensor=2*tf.stop_gradient(g - gbar)*reparam_g), xs=self.pre_temperature_variable)[0]

    variance_objective_grad = tf.reduce_mean(input_tensor=2*(g - gbar)*reinf_g_t) + reparam_g_t
  ...
  def get_rebar_gradient(self):
    ...
    # Construct the variance objective
    variance_objective = tf.reduce_mean(input_tensor=tf.square(U.vectorize(model_grads, set_none_to_zero=True)))
...
class SBNRebar(SBN):
  ...
  def _create_network(self):
    ...
    eta_grads = (self.optimizer_class.compute_gradients(variance_objective,
                                                        var_list=tf.compat.v1.get_collection('CV')))
...
class SBNTrackGradVariances(SBN):
  ...
  def _create_loss(self):
    ...
    mu = tf.reduce_mean(input_tensor=tf.stack([f for f, _ in moments]), axis=0)
    self.grad_variances = []
    deviations = []
    for f, s in moments:
      self.grad_variances.append(tf.reduce_mean(input_tensor=s - tf.square(mu)))
      deviations.append(tf.reduce_mean(input_tensor=tf.square(f - mu)))
    ...
    self.lHat.append(tf.math.log(tf.reduce_mean(input_tensor=mu*mu)))
