...
tf.compat.v1.app.flags.DEFINE_string("working_dir", "/tmp/rebar",
                           """Directory where to save data, write logs, etc.""")
tf.compat.v1.app.flags.DEFINE_string('hparams', '',
                           '''Comma separated list of name=value pairs.''')
tf.compat.v1.app.flags.DEFINE_integer('eval_freq', 20,
...
def manual_scalar_summary(name, value):
  ...
  summary_str = tf.compat.v1.Summary(value=[value])
  return summary_str
...
def train(sbn, train_xs, valid_xs, test_xs, training_steps, debug=False):
  ...
  sv = tf.compat.v1.train.Supervisor(logdir=os.path.join(
