...
class ScheduledSampler(BaseSampler):
  ...
  def _next(self, i):
    ...
    if self._scheduler == 'cycle':
      ...
      return tf.compat.v1.assign(i, tf.math.floormod(i+inc, self._n))
    else:
    ...
    if state is not None and next_state is not None:
      ...
      contexts = tf.concat(
          [0.1 * my_context_range[:self._k] *
           tf.random.normal(tf.shape(input=state[:, :self._k]), dtype=state.dtype) +
           tf.random.shuffle(state[:, :self._k]) - state[:, :self._k],
