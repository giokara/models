...
def bottleneck_layer(x, n, stride, prob, is_training, alpha, beta):
  ...
  with tf.compat.v1.variable_scope('bottleneck_{}'.format(prob)):
    input_layer = x
    ...
    if is_training:
      ...
      random_tensor = prob
      random_tensor += tf.random.uniform(bern_shape, dtype=tf.float32)
      binary_tensor = tf.floor(random_tensor)

      alpha_values = tf.random.uniform(
          [batch_size, 1, 1, 1], minval=alpha[0], maxval=alpha[1],
          dtype=tf.float32)
      beta_values = tf.random.uniform(
