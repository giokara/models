...
def _shake_shake_skip_connection(x, output_filters, stride):
  ...
  path1 = tf.nn.avg_pool2d(
      input=x, ksize=[1, 1, 1, 1], strides=stride_spec, padding='VALID', data_format='NHWC')
  path1 = ops.conv2d(path1, int(output_filters / 2), 1, scope='path1_conv')

  # Skip path 2
  # First pad with 0's then crop
  pad_arr = [[0, 0], [0, 1], [0, 1], [0, 0]]
  path2 = tf.pad(tensor=x, paddings=pad_arr)[:, 1:, 1:, :]
  concat_axis = 3

  path2 = tf.nn.avg_pool2d(
      input=path2, ksize=[1, 1, 1, 1], strides=stride_spec, padding='VALID', data_format='NHWC')
