...
class Inputs(object):
  def __init__(self, config):
    ...
    self.label_smoothing = tf.compat.v1.placeholder(tf.float32, name='label_smoothing')
    self.lengths = tf.compat.v1.placeholder(tf.int32, shape=[None], name='lengths')
    self.mask = tf.compat.v1.placeholder(tf.float32, [None, None], name='mask')
    self.words = tf.compat.v1.placeholder(tf.int32, shape=[None, None], name='words')
    self.chars = tf.compat.v1.placeholder(tf.int32, shape=[None, None, None],
                                name='chars')
