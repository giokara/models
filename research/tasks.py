...
def classification_loss(truth, predicted, weights=None, is_one_hot=True):
  ...
  losses = tf.nn.softmax_cross_entropy_with_logits(
      labels=tf.stop_gradient(truth), logits=predicted)
  if weights is not None:
    losses = tf.boolean_mask(tensor=losses,
                             mask=tf.cast(tf.reshape(weights, [-1]), dtype=tf.bool))
...
class UnrolledTask(object):
  ...
  def tf_episode_batch(self, batch_size):
    ...
    tf_episode_batch = tf.compat.v1.py_func(episode_batch_func, [], touts,
      ...
      if np.any(weights_shape != targets_shape[0:2]):
        raise ValueError('The first two elements of weights shape should match'
                         'target. {} != {}'.format(weights_shape,
                                                   targets_shape))
      true_targets = tf.boolean_mask(tensor=true_targets, mask=weights)
      targets = tf.boolean_mask(tensor=targets, mask=weights)

    return tf.compat.v1.losses.mean_squared_error(tf.reshape(targets, [-1]),
