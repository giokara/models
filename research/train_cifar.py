...
class CifarModel(object):
  ...
  def _setup_images_and_labels(self):
    ...
    self.images = tf.compat.v1.placeholder(tf.float32, [self.batch_size, 32, 32, 3])
  ...
  def _build_graph(self, images, labels, mode):
    ...
    self.predictions, self.cost = helper_utils.setup_loss(
        logits, labels)
    self.accuracy, self.eval_op = tf.compat.v1.metrics.accuracy(
        tf.argmax(input=labels, axis=1), tf.argmax(input=self.predictions, axis=1))
  ...
  def _calc_num_trainable_params(self):
    self.num_trainable_params = np.sum([
        np.prod(var.get_shape().as_list()) for var in tf.compat.v1.trainable_variables()
    ])
    tf.compat.v1.logging.info('number of trainable params: {}'.format(
  ...
  def _build_train_op(self):
    ...
    if hparams.gradient_clipping_by_global_norm > 0.0:
      ...
      tf.compat.v1.summary.scalar('grad_norm', norm)
    ...
    train_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)
...
class CifarModelTrainer(object):
  ...
  def extract_model_spec(self):
    ...
    if checkpoint_path is not None:
      ...
      tf.compat.v1.logging.info('Loaded child model checkpoint from %s',
                      checkpoint_path)
    else:
  ...
  def eval_child_model(self, model, data_loader, mode):
    ...
    """
    tf.compat.v1.logging.info('Evaluating child model in mode %s', mode)
    while True:
      try:
        with self._new_session(model):
          accuracy = helper_utils.eval_child_model(
              ...
              data_loader,
              mode)
          tf.compat.v1.logging.info('Eval child model accuracy: {}'.format(accuracy))
          # If epoch trained without raising the below errors, break
          # from loop.
          break
      except (tf.errors.AbortedError, tf.errors.UnavailableError) as e:
        tf.compat.v1.logging.info('Retryable error caught: %s.  Retrying.', e)
  ...
  def _build_models(self):
    ...
    with tf.compat.v1.variable_scope('model', use_resource=False):
      m = CifarModel(self.hparams)
      m.build('train')
      self._num_trainable_params = m.num_trainable_params
      self._saver = m.saver
    with tf.compat.v1.variable_scope('model', reuse=True, use_resource=False):
    ...
    while True:
      try:
        with self._new_session(m):
          ...
          tf.compat.v1.logging.info('Saving model after epoch')
          self.save_model(step=curr_epoch)
          break
      except (tf.errors.AbortedError, tf.errors.UnavailableError) as e:
        tf.compat.v1.logging.info('Retryable error caught: %s.  Retrying.', e)
    tf.compat.v1.logging.info('Finished epoch: {}'.format(curr_epoch))
    tf.compat.v1.logging.info('Epoch time(min): {}'.format(
  ...
  def _compute_final_accuracies(self, meval):
    ...
    if self.hparams.eval_test:
      ...
      test_accuracy = 0
    tf.compat.v1.logging.info('Test Accuracy: {}'.format(test_accuracy))
  ...
  def run_model(self):
    ...
    with tf.Graph().as_default(), tf.device(
      ...
      tf.compat.v1.logging.info('Before Training Epoch: {}     Val Acc: {}'.format(
      ...
      for curr_epoch in xrange(starting_epoch, hparams.num_epochs):
        ...
        valid_accuracy = self.eval_child_model(
            meval, self.data_loader, 'val')
        tf.compat.v1.logging.info('Epoch: {}    Valid Acc: {}'.format(
