...
class TrainStep(object):
  ...
  def __init__(self,
    ...
    tf.compat.v1.logging.info('Training for %d max_number_of_steps',
...
def create_counter_summaries(counters):
  ...
  if counters:
    with tf.compat.v1.name_scope('Counters/'):
      for name, counter in counters:
        tf.compat.v1.summary.scalar(name, counter)
...
def gen_debug_batch_summaries(batch):
  ...
  with tf.compat.v1.name_scope('batch'):
    for s in range(states.get_shape()[-1]):
      tf.compat.v1.summary.histogram('states_%d' % s, states[:, s])
    for s in range(states.get_shape()[-1]):
      tf.compat.v1.summary.histogram('next_states_%d' % s, next_states[:, s])
    for a in range(actions.get_shape()[-1]):
      tf.compat.v1.summary.histogram('actions_%d' % a, actions[:, a])
    tf.compat.v1.summary.histogram('rewards', rewards)
