...
class SelfAttention(base_layers.BaseLayer):
  ...
  def call(self, inputs, mask, inverse_normalizer, attn_mask=None):
    ...
    if attn_mask is None:
      attn_mask = tf.matmul(mask, tf.transpose(a=mask, perm=[0, 2, 1]))
    ...
    for _ in range(self.num_heads):
      ...
      # for it.
      if self.parameters.mode not in [base_layers.TFLITE, base_layers.PREDICT]:
        queries = tf.transpose(a=queries, perm=[0, 2, 1])
        ...
        queries = tf.transpose(a=queries)
...
class SelfAttentionV2(base_layers.BaseLayer):
  ...
  def call(self, inputs, mask, inverse_normalizer, attn_mask=None):
    ...
    invalid_mask = (1 - attn_mask) * self.parameters.invalid_logit
    if self.parameters.mode not in [base_layers.TFLITE, base_layers.PREDICT]:
      queries = tf.transpose(a=tensors[0], perm=[0, 2, 1, 3])
      keys = tf.transpose(a=tensors[1], perm=[0, 2, 1, 3])
      values = tf.transpose(a=tensors[2], perm=[0, 2, 1, 3])
      ...
      result = tf.transpose(a=result, perm=[0, 2, 1, 3])
...
class TransformerEncoder(base_layers.BaseLayer):
  ...
  def __init__(self,
    ...
    self.model_dimension = model_dimension
    self.parameters.initializer = tf.compat.v1.keras.initializers.TruncatedNormal(
...
class TransformerEncoderStack(base_layers.BaseLayer):
  ...
  def call(self, input_indices, sequence_length):
    mask_rank2 = tf.sequence_mask(
        sequence_length, tf.shape(input=input_indices)[1], dtype=tf.float32)
    mask_rank3 = tf.expand_dims(mask_rank2, axis=2)
    inverse_normalizer = tf.math.reciprocal(tf.reduce_sum(input_tensor=mask_rank3))
    if self.parameters.mode in [base_layers.PREDICT, base_layers.TFLITE]:
      sequence_length = tf.reduce_sum(input_tensor=input_indices + 1 - input_indices)
    ...
    attn_mask = tf.matmul(mask_rank3, tf.transpose(a=mask_rank3, perm=[0, 2, 1]))
...
class TransformerEncoderStackWithInputEmbedding(TransformerEncoderStack):
  ...
  def call(self, inputs, sequence_length):
    mask_rank2 = tf.sequence_mask(
        sequence_length, tf.shape(input=inputs)[1], dtype=tf.float32)
    mask_rank3 = tf.expand_dims(mask_rank2, axis=2)
    inverse_normalizer = tf.math.reciprocal(tf.reduce_sum(input_tensor=mask_rank3))
    attn_mask = tf.matmul(mask_rank3, tf.transpose(a=mask_rank3, perm=[0, 2, 1]))
    if self.parameters.mode not in [base_layers.PREDICT, base_layers.TFLITE]:
...
class FunnelAttention(base_layers.BaseLayer):
  ...
  def call(self, inputs, mask, inverse_normalizer, memory, memory_mask,
    ...
    invalid_mask = (1 - attn_mask) * self.parameters.invalid_logit
    if self.parameters.mode not in [base_layers.TFLITE, base_layers.PREDICT]:
      queries = tf.transpose(a=q_tensor, perm=[0, 2, 1, 3])
      keys = tf.transpose(a=kv_tensors[0], perm=[0, 2, 1, 3])
      values = tf.transpose(a=kv_tensors[1], perm=[0, 2, 1, 3])
...
class FunnelTransformerEncoderStack(base_layers.BaseLayer):
  ...
  def call(self, inputs, sequence_length):
    ...
    pooled_inverse_normalizer = tf.math.reciprocal(tf.reduce_sum(input_tensor=pooled_mask))
...
class DecoderUniformAttention(base_layers.BaseLayer):
  ...
  def get_uniform_attention(self, attn_mask=None):
    ...
    mask = tf.math.divide_no_nan(
        self.causal_mask,
        tf.reduce_sum(input_tensor=self.causal_mask, axis=-1, keepdims=True))
