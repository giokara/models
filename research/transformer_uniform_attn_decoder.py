...
class TransformerUniformAttnDecoderStack(base_layers.BaseLayer):
  ...
  def call(self,
    ...
    dec_inverse_normalizer = tf.math.reciprocal(tf.reduce_sum(input_tensor=dec_mask_rank3))
    enc_mask_rank3 = tf.expand_dims(enc_mask, 1)
    enc_inverse_normalizer = tf.math.reciprocal(tf.reduce_sum(input_tensor=enc_mask_rank3))
...
class Model(tf.keras.layers.Layer):
  ...
  def __init__(self, config, mode):
    ...
    self.embedding = embedding_layers.EmbeddingFullyConnected(
        ...
        initializer=tf.compat.v1.random_uniform_initializer(-math.sqrt(3), math.sqrt(3)),
        parameters=self.parameters)
  ...
  def call(self,
    ...
    if self.parameters.mode == base_layers.TRAIN:
      ...
      input_mask = tf.ones(tf.shape(input=input_values)[:-1], dtype=tf.float32)
  ...
  def model_outputs(self, layer_in):
    bsz = layer_in.get_shape().as_list()[0] or tf.shape(input=layer_in)[0]
    ...
    predicted_ids = tf.argmax(input=logits, axis=2, output_type=tf.int64)
    return logits, predicted_ids
  ...
  def decode_beam_search(self,
                         ...
                         enc_mask,
                         scope="model"):
    batch_size = tf.shape(input=start_ids)[0]
    ...
    def symbols_to_logits_fn(ids, step, cache):
      ...
      with tf.compat.v1.name_scope(scope):
        ...
        input_mask = tf.ones(tf.shape(input=curr_embed)[:-1], dtype=tf.float32)
        if self.embedding_size != self.model_dimension:
    ...
    beam_ids = tf.pad(
        tensor=beam_ids, paddings=[[0, 0], [0, self.max_dec_time_step - tf.shape(input=beam_ids)[1]]],
        constant_values=pad_id)
    logits = states["logits"][:, 0, :, :]
    logits = tf.pad(
        tensor=logits,
        paddings=[[0, 0], [0, self.max_dec_time_step - tf.shape(input=logits)[1]], [0, 0]],
