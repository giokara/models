...
def softplus(x):
  ...
  return m + tf.math.log(tf.exp(-m) + tf.exp(x - m))
...
def center(x):
  mu = (tf.reduce_sum(input_tensor=x) - x)/tf.cast(tf.shape(input=x)[0] - 1, dtype=tf.float32)
  return x - mu
...
def binary_log_likelihood(y, log_y_hat):
  ...
  Returns:
    log_likelihood
  """
  return tf.reduce_sum(input_tensor=y*(-softplus(-log_y_hat)) +
                       (1 - y)*(-log_y_hat-softplus(-log_y_hat)),
                       axis=1)

def cov(a, b):
  """Compute the sample covariance between two vectors."""
  mu_a = tf.reduce_mean(input_tensor=a)
  mu_b = tf.reduce_mean(input_tensor=b)
  n = tf.cast(tf.shape(input=a)[0], dtype=tf.float32)

  return tf.reduce_sum(input_tensor=(a - mu_a)*(b - mu_b))/(n - 1.0)

def corr(a, b):
  return cov(a, b)*tf.math.rsqrt(cov(a, a))*tf.math.rsqrt(cov(b, b))
...
def logSumExp(t, axis=0, keep_dims = False):
  ...
  '''
  m = tf.reduce_max(input_tensor=t, axis=[axis])
  res = m + tf.math.log(tf.reduce_sum(input_tensor=tf.exp(t - tf.expand_dims(m, axis)), axis=[axis]))
